...
if [ -z "${GITHUB_STEP_SUMMARY}" ] || [ -z "${GITHUB_OUTPUT}" ]; then
...
# Current Versions
UPSTREAM="almalinux"
CURRENT_IMAGE=$(grep -Eo "^FROM ${UPSTREAM}:([^ ]+)" "${DOCKERFILE}" | awk -F: '{print $2}')
CURRENT_SRVADMIN=$(grep -Eo "srvadmin-all[^ ]*" "${DOCKERFILE}")
CURRENT_DSU=$(grep -Eo "dell-system-update[^ ]*" "${DOCKERFILE}")
...
if [ -z "${LATEST_IMAGE}" ] || [ -z "${LATEST_SRVADMIN}" ] || [ -z "${LATEST_DSU}" ]; then
