...
RUN sed -i 's/enabled=0/enabled=1/' /etc/yum.repos.d/almalinux-crb.repo && \
    ...
    dnf -y install srvadmin-all-11.0.0.0-5268.el9 dell-system-update && \
    dnf clean all && \
    rm -Rfv /usr/lib/systemd/system/autovt@.service /usr/lib/systemd/system/getty@.service /tmp/bootstrap.sh-4c713e348603a5ce0231667b24a42907 /tmp/copygpgkeys.sh
